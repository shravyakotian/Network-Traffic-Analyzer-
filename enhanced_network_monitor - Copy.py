                        ...
                        with self.connection_lock:
                            if conn_id not in self.seen_connections:
                                ...
                                if domain:
                                    self.websites_visited.add(domain)
                                    self.domains_visited.add(domain)
                                    self.log_activity("NETWORK", f"{process_name} -> {domain} ({remote_ip}:{remote_port}) [{protocol}]")
                                else:
                                    self.log_activity("NETWORK", f"{process_name} -> {remote_ip}:{remote_port} [{protocol}]")
...
def main():
    """Main function"""
    print("🚀 Enhanced Network Traffic Monitor for Windows")
    print("📋 Advanced monitoring for modern browser traffic including QUIC/HTTP3")
    print("🔍 Multi-method detection: DNS, Process monitoring, Packet capture")
    ...
    try:
        ...
        if os.name == 'nt' and not ctypes.windll.shell32.IsUserAnAdmin():
            ...
            print("   Process and connection monitoring will still work")
            print()
    except:
        pass
